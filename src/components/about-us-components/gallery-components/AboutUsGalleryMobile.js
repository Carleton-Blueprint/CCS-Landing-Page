...
const AboutUsGalleryMobile = (props) => {
    ...
    return (
        <div className='flex flex-col'>
            {
                allImages.map((image,index) =>{
                    return(
                        <div className={` transition-opacity duration-200 ease-in-out ${visibleIndex !== index ? 'p-0 w-0 h-0 opacity-0' : 'px-10 h-44 opacity-100' }`}>
                            {
                                visibleIndex === index && 
                                <GalleryImage key={image.galleryImage.id} url={image.galleryImage.gatsbyImageData} description={image.galleryImage.description} index = {index}/>
                            }    
                        </div>
                    )
                })
        ...
        <div className='flex justify-center py-10 gap-10'>
            ...
            onClick={() => setVisibleIndex((prev) => (prev-1 === -1) ? imagesLength - 1 : prev-1)}
