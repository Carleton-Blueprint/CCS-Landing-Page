...
const countCharactersOfContentfulRichTextNode = (contentfulRichTextNode) => {
  ...
  const paragraphs = contentfulRichTextNode.filter(
    (item) => item.nodeType === 'paragraph'
  );

  for (let paragraph of paragraphs) {
    characterCount += paragraph.content.reduce(
      (total, item) => total + (item.value ? item.value.length : 0),
      0
    );
  }
...
const EventBlock = (props) => {
  ...
  const getSize = () => {
    return countCharactersOfContentfulRichTextNode(description.content) > 250
      ? 'text-sm'
      : 'text-base';
  };
